...
class Transfussion(nn.Module):
    def __init__(self, model:nn.Module, config:tp.Any):
        super().__init__()
        self.model = model
        self.embeddings = TextOps(config) # text embedding
        self.patch_ops = PatchOps(config) # image embedding
        self.lm_emb_linear = nn.Linear(config.d_model, config.lm_output_units) # text embedding linear
        self.img_patch_linear = nn.Linear(config.d_model, (config.patch_size**2)*config.out_channels) # image embedding linear
        ...
        self.txt_maxlens = config.text_maxlen # context length
    ...
    def _compute_mask(self, mask_params:list[tuple[int, int]]):
        ...
        for (from_, num_image_tokens) in mask_params:
            mask[:, :, from_:from_+num_image_tokens, from_:from_+num_image_tokens] = True # b, head, h, w
    ...
    def forward(
        ...
        inputs:Tensor = torch.cat(inputs, dim=1) # (B, T, D) # list -> torch.tensor
        outputs = self.model(inputs, self._compute_mask(mask_params)) # (B, T, D)

        # Split the output back into the different modalities
        outputs = torch.split(outputs, modality_lengths, dim=1) # modalities split

        # Apply the linear layers to the different modalities
        modality_token_emb:list[Tensor] = [] # Decoded token embeddings by modality
